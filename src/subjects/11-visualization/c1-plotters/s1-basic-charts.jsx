...
export default function BasicCharts() {
  return (
    <div className="prose-rust">
      <h1>Basic Charts with Plotters</h1>

      <p>
        Python's matplotlib is the gold standard for data visualization.
        Rust's <code>plotters</code> crate provides similar charting capabilities
        with a builder-pattern API. While not as mature as matplotlib, plotters
        can generate publication-quality charts as PNG, SVG, or even render
        to a WebAssembly canvas.
      </p>

      <ConceptBlock title="Plotters Overview">
        <p>
          <code>plotters</code> uses a layered approach: you create a drawing
          backend (PNG, SVG, or HTML canvas), then build a chart context on top,
          then draw series onto it. The API favors method chaining and Rust's
          type system ensures you cannot draw incompatible data types on the
          same axis.
        </p>
      </ConceptBlock>

      <h2>Line Chart: matplotlib vs plotters</h2>

      <PythonRustCompare
        title="Drawing a simple line chart"
        description="Both libraries follow a similar workflow: create figure, set up axes, plot data, save."
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    Ok(())
      ...
      <NoteBlock type="pythonista" title="Builder pattern instead of pyplot">
        matplotlib uses a stateful API (<code>plt.plot()</code>,{' '}
        <code>plt.title()</code>). Plotters uses the builder pattern: you
        chain method calls to configure the chart, then draw data onto it.
        This is more verbose but less error-prone — the compiler catches
        type mismatches between your data and axis configuration.
      </NoteBlock>
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    Ok(())
      ...
      <NoteBlock type="tip" title="SVG for vector output">
        Replace <code>BitMapBackend::new</code> with{' '}
        <code>SVGBackend::new("chart.svg", (800, 600))</code> for scalable
        vector graphics — perfect for papers and presentations. The API is
...
plotters = "0.3"`}
      ...
      <ExerciseBlock
        ...
        problem={`Create a line chart showing a simulated training loss curve:
...
Use f64::exp() for the exponential.`}
        solution={`use plotters::prelude::*;
